...
int write_inode(unsigned int _inode_seq, struct stat *stats, unsigned int page_offset, int father_inode_num, char *name)
{
	...
	inode_offset = (_inode_seq + 1) * INODE_SIZE;//考虑开头的SuperBlock,inode_seq从1位置开始写。
				    ...
				    S_ISDIR(stats.st_mode) )  //检查文件类型，是不是能够处理的类型
				{
					...
					if(S_ISREG(stats.st_mode))
					{
						...
						if(h >= 0)
						{
							...
							printf("%s:%d data chunks written\n", entry->d_name, data_page_offset - first_page);
							close(h);
					...
					else if(S_ISDIR(stats.st_mode))
					{
						...
						if (write_inode(inode_seq, &stats, 0, parent, entry->d_name) == 0) {
							...
							ret = process_directory(inode_seq,full_name);
							if(ret < 0)
								goto out;
						}
					}
